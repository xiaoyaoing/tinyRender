...
vec<N,T> cwiseProduct(vec<N,T> a,vec<N,T> b){
    vec<N,T> c;
    for(int i=0;i<N;i++)
        c[i]=a[i]*b[i];
    return c;
...
template <class T>
T linearInterpolation(T val1,T val2,double proportion){
    return val1*proportion+val2*(1-proportion);
}

template <class T>
T linearInterpolation(T val1,T val2,T val3,double rate1,double rate2,double  rate3){
    return val1*rate1+val2*rate2+val3*rate3;
}
template <class T>
T linearInterpolation(T val1,T val2,T val3,double rate1,double rate2,double  rate3,double  weight){
    return linearInterpolation(val1,val2,val3,rate1,rate2,rate3)*weight;
}
